...
export default {
  ...
  watch: {
    ...
    targetAddress: async function(val) {
      try {
        ...
        console.error(e);
        window.localStorage.setItem('target-address', 'undefined');
      }
    },
  },
  ...
  methods: {
    ...
    async getChecksummedAddress() {
      return ethers.utils.getAddress(this.targetAddress);
    },

    /**
     * @notice Returns the wallet instance for the mnemonic in local storage
     */
    getEphemeralWallet() {
    ...
    /**
     * @notice Returns the address for the mnemonic in local storage
     */
